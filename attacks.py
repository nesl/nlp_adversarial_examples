...
class GeneticAtack(object):
    ...
    def perturb(self, x_cur, x_orig, target):
        # Pick a word that is not modified and is not UNK
        x_len = np.sum(np.sign(x_cur))
        to_modify = [idx  for idx in range(x_len) if (x_cur[idx] == x_orig[idx] and self.inv_dict[x_cur[idx]] != 'UNK' and 
                                                     self.dist_mat[x_cur[idx]][x_cur[idx]] != 100000)]
        rand_idx = np.random.choice(to_modify)
        src_word = x_cur[rand_idx]
        replace_list,_ =  glove_utils.pick_most_similar_words(src_word, self.dist_mat, self.pop_size)
        replace_list = [w if w != 0 else src_word for w in replace_list]
        return self.select_best_replacement(rand_idx, x_cur, x_orig, target, replace_list)
        
    def generate_population(self, x_orig, target, pop_size):
        return [self.perturb(x_orig, x_orig, target) for _ in range(pop_size)]
    ...
    def attack(self, x_orig, target, max_change=0.4):
        x_adv = x_orig.copy()
        pop = self.generate_population(x_orig, target, self.pop_size)
        for i in range(self.max_iters):
            ...
            childs = [self.perturb(x, x_orig, target) for x in childs]
            pop = elite + childs 
